  ...
  useEffect(() => {
    const fetchSummaryData = async () => {
      try {
        ...
        let incomesQuery;
        if (isAdmin) {
          incomesQuery = query(collection(db, 'incomes'));
        } else {
          incomesQuery = query(collection(db, 'incomes'), where('userId', '==', user?.uid));
        }
        ...
        incomesSnapshot.forEach((doc) => {
          const data = doc.data();
          totalEstimated += data.estimatedAmount;
        ...
        if (isAdmin) {
          ...
          expensesQuery = query(collection(db, 'expenses'), where('userId', '==', user?.uid));
        }
        ...
        expensesSnapshot.forEach((doc) => {
          const data = doc.data();
          totalExpenses += data.debitedAmount;
